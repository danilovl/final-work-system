...
readonly class CloneEventScheduleHandler implements CommandHandlerInterface
{
    public function __construct(private EventScheduleFactory $eventScheduleFactory) {}

    public function __invoke(CloneEventScheduleCommand $command): void
    {
        $this->eventScheduleFactory->cloneEventSchedule(
            $command->user,
            $command->eventSchedule,
            $command->eventScheduleCloneModel->start
        );
    }
}
